    ...
    class Keyring
    {
        ...
        public function add ( $serviceName, $serviceHost, $serviceKey )
        {
            for ( $i = 0; $i < count( $this->keyring ); $i++ )
            {
                ...
                {
                    $this->keyring[$i] = array( 'serviceName' => $serviceName, 'serviceHost' => $serviceHost, 'serviceKey' => md5( $serviceKey ) );
                    return;
                }
            }
            
            $this->keyring[] = array( 'serviceName' => $serviceName, 'serviceHost' => $serviceHost, 'serviceKey' => md5( $serviceKey ) );
        ...
        public function check ( $serviceName, $serviceHost, $serviceKeyToCheck )
        {
            for ( $i = 0; $i < count( $this->keyring ); $i++ )
            {
                if ( $this->keyring[$i]['serviceName'] == $serviceName
                    && $this->keyring[$i]['serviceHost'] == $serviceHost
                    && $this->keyring[$i]['serviceKey'] == md5($serviceKeyToCheck) )
                {
                    return true;
                }
            }
