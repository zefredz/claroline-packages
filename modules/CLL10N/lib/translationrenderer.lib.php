...
class TranslationRenderer{
  ...
  public static function moduleProgression( $moduleLabel, $progression )
  {
    ...
    $cmdMenu[] = claro_html_cmd_link( claro_htmlspecialchars( Url::Contextualize( $_SERVER['PHP_SELF'] ) ), '<img src="' . get_icon_url('translate') . '" alt="" />' . get_lang('Go back to the list'));
    $cmdMenu[] = claro_html_cmd_link( claro_htmlspecialchars( Url::Contextualize( $_SERVER['PHP_SELF'] . '?cmd=rqGenerate&module=' . $moduleLabel ) ), '<img src="' . get_icon_url('translate_add') . '" alt="" />' . get_lang('Generate language files'));
