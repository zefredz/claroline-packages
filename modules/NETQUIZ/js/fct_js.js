...
function openStats(id) {
	...
	if (!winQuiz || winQuiz.closed){
		...
		var settings = "status,scrollbars,menubar,resizable,width="+largeur+",height="+hauteur+",top="+wint+",left="+winl;
		winQuiz = window.open( "netquiz/viewquizstats.php?id=" + id,"entree_netquiz",settings );
	} else {
		winQuiz.focus()
	}
}
...
function showDetailQuestion(iIDQuestion, iIDQuiz, statCurrentUser)
{
    openWindowsAndCenter( "dsp_view_all_question_detail.php?idquiz=" + iIDQuiz + "&idquestion=" + iIDQuestion + "&statCurrentUser=" + statCurrentUser,770,570 );
}
function showDetailParticipant(iIDParticipant, iIDQuiz)
{
    openWindowsAndCenter( "dsp_view_all_participant_detail.php?idquiz=" + iIDQuiz + "&idparticipant=" + iIDParticipant,770,570 );
}

function editStatusQuestion(iIDQuestion, iActive)
{
    openWindowsAndCenterNSB( "act_edit_status_question.php?id=" + iIDQuestion + "&a=" + iActive,300,150,"editstatus" );
}
function editPointageQuestion(iIDP, iP, iIDQuestion)
{
    iScrollPos = getScrollPos();
    openWindowsAndCenterNSB( "act_edit_pointage.php?idq=" + iIDQuestion + "&idp=" + iIDP + "&p=" + iP,300,150,"editpointage" );
}

function editStatusParticipant(iIDParticipant, iActif)
{
    openWindowsAndCenterNSB( "act_edit_status_participant.php?id=" + iIDParticipant + "&a=" + iActif,300,150,"editstatus" );
}
function editPointageParticipant(iIDQ, iP, iIDParticipant)
{
    openWindowsAndCenterNSB( "act_edit_pointage.php?idq=" + iIDQ + "&idp=" + iIDParticipant + "&p=" + iP,300,150,"editpointage" );
}

function exportParticipations(iIDQuiz, sOrderByID, sOrderByDirection, statCurrentUser)
{
    //openWindowsAndCenter("act_edit_status.php?id=" + iIDQuiz + "&status=" + sStatut,300,150);
    //window.open( "exportparticipations.php?id=" + iIDQuiz + "&ob=" + sOrderByID + "&obd=" + sOrderByDirection );
    document.location.href = ( "act_exportparticipations.php?id=" + iIDQuiz + "&ob=" + sOrderByID + "&obd=" + sOrderByDirection + "&statCurrentUser=" + statCurrentUser );
}

function exportQuestions(iIDQuiz, sOrderByID, sOrderByDirection, statCurrentUser)
{
    //openWindowsAndCenter("act_edit_status.php?id=" + iIDQuiz + "&status=" + sStatut,300,150);
    //window.open( "exportparticipations.php?id=" + iIDQuiz + "&ob=" + sOrderByID + "&obd=" + sOrderByDirection );
    document.location.href = ( "act_exportquestions.php?id=" + iIDQuiz + "&ob=" + sOrderByID + "&obd=" + sOrderByDirection + "&statCurrentUser=" + statCurrentUser );
}
