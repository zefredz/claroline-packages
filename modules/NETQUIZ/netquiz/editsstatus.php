...
### fin debug #################################################################################################
    ...
    // Class netquiz : update du status des participants
	$netquiz->setIdParticipant( $iIDParticipant );
	$netquiz->setParticipantsActif( $iActif );

	if ( !$netquiz->updateParticipantsStatus() )
	{
    ...
    $sQuery = "update nq_participants set Actif = $iActif where IDParticipant = $iIDParticipant";
    ...
    if($bCloseWindow){
        ...
        <html>
            <head>
                ...
                <title><?php echo $sLR["qp_sta_lbl"]; ?></title>
                <script>
                    function cancel(){
                        window.close();
                    }
            ...
            <body>
                <form method="get" action="editsstatus.php">
                    <table width="100%" cellpadding="0" cellspacing="0" border="0">
                        <tr>
                            <td align="left"><strong><?php echo $sLR["qp_sta_lbl"]; ?></strong></td>
                        </tr>
                        <tr>
                            <td>&nbsp;</td>
                        </tr>
                        <tr>
                            <td align="left">
                                <input type="radio" name="a" value="1" id="a1" <?php echo ($iActif == 0 ? "" : " checked");?> /><label for="a1">&nbsp;&nbsp;<?php echo $sLR["qp_staa_lbl"]; ?></label><br />
                                <input type="radio" name="a" value="0" id="a0" <?php echo ($iActif == 1 ? "" : " checked");?> /><label for="a0">&nbsp;&nbsp;<?php echo $sLR["qp_staia_lbl"]; ?></label>
                                <input type="hidden" name="id" value="<?php echo $iIDParticipant; ?>" />
