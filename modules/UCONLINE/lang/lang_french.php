<?php

$_lang['1 user connected'] = '1 utilisateur en ligne';
$_lang['%countOfUsers users connected'] = '%countOfUsers utilisateurs en ligne';
$_lang['Cannot save change.'] = 'Impossible d\'enregistrer les modifications.';
$_lang['Configure your Skype account'] = 'Configurer votre compte Skype';
$_lang['Do not forget to allow your status to be shown from your Skype client.'] = 'N\'oubliez pas d\'autoriser l\'affichage de votre statut dans votre logiciel Skype.';
$_lang['Leave empty to deactivate status notifier.'] = 'Laissez ce champs vide pour désactiver la notification.';
$_lang['List of active users for the last %time minutes :'] = 'Liste des utilisateurs actifs depuis au moins %time minutes :';
$_lang['Local Time'] = 'Heure locale';
$_lang['No user connected'] = 'Aucun utilisateur connecté';
$_lang['Skype account'] = 'Compte Skype';
$_lang['Skype name'] = 'Nom Skype';
$_lang['Skype name successfully changed.'] = 'Adresse Skype modifiée avec succès.';
$_lang['Skype options, Privacy panel.'] = 'Options Skype, Confidentialité.';
$_lang['Skype status notifier successfully deactivated.'] = 'La notification de statut Skype a été désactivée.';
$_lang['Skype status notifier'] = 'Notification de statut Skype';
$_lang['User(s) online'] = 'Utilisateurs connectés';
$_lang['user picture'] = 'photo utilisateur';
$_lang['user\'s Skype status'] = 'Status Skype de l\'utilisateur';
$_lang['Warning : you can only see the users registered in YOUR courses!'] = 'Attention! Vous ne pouvez voir que les étudiants inscrits dans les mêmes cours que vous!';