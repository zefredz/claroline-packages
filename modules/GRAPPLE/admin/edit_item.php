<?php // $Id$
/**
 * Edition of a resource in a learning path.
 * 
 * This page is used to manage a resource.
 * The title, description, completion threshold and branching conditions are setted here.
 *
...
/*
 * Tool libraries
 * Load require librairies to manage the resource (path.class.php, item.class.php, linker.inc.php)
 */
 ...
 * Command exEdit
 *
 * Set item parameters (title, description, ...), check if the item is valide and then save it. *
 */
if( $cmd == 'exEdit' )
{
    $error = false;
    ...
    if( $item->validate() && !$error )
    {
        if( $insertedId = $item->save() )
        {
            ...
            $eventNotifier->notifyCourseEvent("grapple_resource_created",claro_get_current_course_id(), claro_get_current_tool_id(), $itemId, claro_get_current_group_id(), claro_get_current_user_id() );
        }
        else
        {
